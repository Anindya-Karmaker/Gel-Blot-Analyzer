...
if __name__ == "__main__":
    ...
    try:
        ...
        class GlycosylationMapperDialog(QDialog):
            ...
            and calculating potential molecular weights of glycosylated fragments.
            """
            def __init__(self, sequence, base_mw, glycan_mass, parent=None):
                super().__init__(parent)
                self.setWindowTitle("Glycosylation Mapper")
                ...
                form_layout.addWidget(QLabel("Potential Fragments (kDa):"), 5, 0, Qt.AlignTop)
                self.potential_fragments_text = QTextEdit()
                self.potential_fragments_text.setReadOnly(True)
                self.potential_fragments_text.setFixedHeight(80)
                form_layout.addWidget(self.potential_fragments_text, 5, 1)
            ...
            def process_sequence(self):
                ...
                try:
                    ...
                    if self.num_glycosylation_sites > 0:
                        ...
                        result_text += f"\n\nTOTAL POTENTIAL SITES: {self.num_glycosylation_sites}"
            ...
            def update_potential_fragments(self):
                try:
                    ...
                    if base_mw <= 0 or glycan_mass < 0 or self.num_glycosylation_sites < 0:
                        raise ValueError
                    
                    fragments = [f"Unglycosylated (0 glycans): {base_mw:.2f} kDa"]
                    for i in range(1, self.num_glycosylation_sites + 1):
                        potential_mw = base_mw + (i * glycan_mass)
                        fragments.append(f"Protein + {i} glycan(s): {potential_mw:.2f} kDa")
                    self.potential_fragments_text.setPlainText("\n".join(fragments))
                except (ValueError, TypeError):
                    self.potential_fragments_text.clear()

            def get_results(self):
                """Returns the final state of the dialog's inputs and calculations."""
                try:
                    base_mw = float(self.base_protein_mw_input.text())
                except ValueError:
                    base_mw = 0.0
        ...
        class CombinedSDSApp(QMainWindow):
            ...
            def __init__(self):
                ...
                self.window_title="GEL BLOT ANALYZER V1.00"
                self.protein_sequence = ""
                self.base_protein_mw = 0.0
                self.avg_glycan_mass = 0.0
                self.num_glycosylation_sites = 0
                self.glycosylation_products = [] # List of calculated potential MWs
           ...
           # --- END: Modified Warping ---
            ...
            def analysis_tab(self):
                ...
                self.open_glyco_mapper_button = QPushButton("Open Glycosylation Mapper...")
                ...
                self.open_glyco_mapper_button.clicked.connect(self.open_glycosylation_mapper)
            ...
            def open_glycosylation_mapper(self):
                ...
                if dialog.exec() == QDialog.Accepted:
                    ...
                    # Recalculate and store potential products
                    self.glycosylation_products = []
                    if self.base_protein_mw > 0 and self.avg_glycan_mass > 0 and self.num_glycosylation_sites > 0:
                        for i in range(self.num_glycosylation_sites + 1):
                            self.glycosylation_products.append(self.base_protein_mw + (i * self.avg_glycan_mass))
            ...
            def get_protein_location(self, event, all_marker_positions, all_marker_values):
                ...
                self.last_mw_prediction_coeffs = coefficients
                self.last_mw_prediction_min_max_pos = (min_pos_active, max_pos_active)
